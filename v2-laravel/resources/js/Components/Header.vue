...
<template>
    ...
    >
            ...
            <div class="flex items-center justify-between py-4 gap-6">
                ...
                <nav class="hidden md:flex items-center gap-8">
                    ...
                    <template v-if="auth?.user">
                        <div class="relative">
                            ...
                            >
                                <div class="w-8 h-8 rounded-full bg-gradient-to-br from-primary to-purple flex items-center justify-center text-white text-xs font-bold">
                            ...
                            >
                                <div v-if="isUserMenuOpen" class="absolute right-0 mt-2 w-48 bg-white rounded-xl shadow-xl border border-bg-alt py-2 z-50">
                    ...
                    <Link
                        v-else
                        href="/login"
                        class="inline-flex items-center gap-2 bg-gradient-to-r from-primary to-primary-dark text-white px-5 py-2.5 rounded-full text-sm font-semibold shadow-md shadow-primary/25 hover:shadow-lg hover:shadow-primary/30 hover:-translate-y-0.5 transition-all duration-300"
                    >
                        <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                ...
                <div class="flex md:hidden items-center gap-2">
                    ...
                    <template v-if="auth?.user">
                        <button
                            @click="isUserMenuOpen = !isUserMenuOpen"
                            class="w-10 h-10 flex items-center justify-center rounded-full bg-gradient-to-br from-primary to-purple text-white text-sm font-bold"
                        >
                            {{ auth.user.name.charAt(0).toUpperCase() }}
                        </button>
                    </template>
            ...
            <div v-if="isUserMenuOpen && auth?.user" class="md:hidden bg-white border-t shadow-lg">
                <div class="max-w-7xl mx-auto px-5 py-4">
                    <div class="flex items-center gap-3 mb-3 pb-3 border-b border-bg-alt">
                        <div class="w-10 h-10 rounded-full bg-gradient-to-br from-primary to-purple flex items-center justify-center text-white font-bold">
                            {{ auth.user.name.charAt(0).toUpperCase() }}
                        </div>
                        <div>
        ...
        <!-- MOBILE MENU (only nav links, NOT login) -->
        <Transition
